 ...
 * $Id: mivapi.h 66168 2017-12-06 00:23:10Z dcarver $
 */
...
void				mvProgram_SetProcessName( mvProgram program, const char *name, int name_length );
...
const char *		mvProgram_GetCharacterSet( mvProgram program );
int					mvProgram_CharacterSetIsUTF8( mvProgram program );
...
void				mvProgram_Asynchronous_Sleep( mvProgram program, time_t until, int semlocation, const char *sempath, int sempath_length );

int					mvProgram_FatalError( mvProgram program, const char *error, int error_length );

/*
 * mvLinkBuffer functions
 */

mvLinkBuffer		mvLinkBuffer_Allocate( int first_block_size, int grow_size );
void				mvLinkBuffer_Free( mvLinkBuffer linkbuffer );
void				mvLinkBuffer_Concat_String( mvLinkBuffer linkbuffer, const char *buffer, int buffer_length );
void 				mvLinkBuffer_Concat_String_Nocopy( mvLinkBuffer linkbuffer, const char *buffer, int buffer_length, int buffer_size );
void				mvLinkBuffer_Concat_Char( mvLinkBuffer linkbuffer, char data );
void 				mvLinkBuffer_Concat_HexChar( mvLinkBuffer linkbuffer, unsigned char c );
void 				mvLinkBuffer_Concat_Int( mvLinkBuffer linkbuffer, int value );
void 				mvLinkBuffer_Concat_Double( mvLinkBuffer linkbuffer, double value );
void 				mvLinkBuffer_Concat_Variable( mvLinkBuffer linkbuffer, mvVariable variable );
char *				mvLinkBuffer_Release( mvLinkBuffer linkbuffer, int *data_length, int *data_size, int headroom );
const char *		mvLinkBuffer_Data( mvLinkBuffer linkbuffer, int *data_length );
...
#define MVVTYPE_NONE		0
#define MVVTYPE_INTEGER		1
#define MVVTYPE_DOUBLE		2
#define MVVTYPE_STRING		3
#define MVVTYPE_STRUCTURE	4
#define MVVTYPE_ARRAY		5
...
char *				mvVariable_Value_NS( mvVariable variable, int *length );
...
int					mvVariable_Type( mvVariable variable );
...
int					mvVariable_Array_Collapse( mvVariable agg );
...
int					mvVariableHash_Entries( mvVariableHash hash );
...
mvHTTPResponse		mvNetwork_HTTP_Request( mvProgram program, const char *method, const char *url, double http_version, const char *content, const char *contenttype, int contentlength );

int					mvHTTPResponse_Status_Code( mvHTTPResponse response );
int					mvHTTPResponse_Content_Length( mvHTTPResponse response );
int					mvHTTPResponse_Read( mvHTTPResponse response, char *buffer, int size );
void				mvHTTPResponse_Close( mvHTTPResponse response );
const char *		mvHTTPResponse_Error( mvProgram program, mvHTTPResponse response );
...
char *				mvFile_NormalizePath( mvProgram program, int location, const char *path, int path_length, int *normalized_length, int *normalized_size );
...
int					mvFile_TouchFile( mvProgram program, int location, const char *filename, int filename_length );
...
#define MvCONFIG_FILE_CA					0x02000033
...
#define MvCONFIG_TIMEOUT_ASYNCHRONOUS		0x01000031
#define MvCONFIG_TIMEOUT_ASYNCHRONOUS_SLEEP	0x01000032
...
#define MvCONFIG_MAX_ASYNCHRONOUS_TASKS		0x01000030
...
#define MvCONFIG_SSL_CONFIG					0x0200001d
#define MvCONFIG_SSL_SNI					0x01000029
